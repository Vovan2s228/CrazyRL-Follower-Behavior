...
class NormalizeVecRewEnvState:
	mean: jnp.ndarray  # shape (batch_size,)
	var: jnp.ndarray   # shape (batch_size,)
	count: jnp.ndarray  # shape (batch_size,)
	return_val: jnp.ndarray  # shape (batch_size,)
	env_state: State
...
class NormalizeVecReward(Wrapper):
	...
	def reset(self, key: chex.PRNGKey) -> Tuple[chex.Array, dict, NormalizeVecRewEnvState]:
		...
		return obs, info, NormalizeVecRewEnvState(
			mean=jnp.zeros((batch_size,)),
			var=jnp.ones((batch_size,)),
			count=jnp.full((batch_size,), 1e-4),
			return_val=jnp.zeros((batch_size,)),
			env_state=state,
		)

	def step(
		self, state: NormalizeVecRewEnvState, action: chex.Array, key: chex.Array
		...
		done = jnp.logical_or(jnp.any(term), jnp.any(truncated))  # (batch_size,)
		
		# Sum agent rewards into a team reward per env (shape: (batch_size,))
		team_reward = reward.sum(axis=1)

		return_val = state.return_val * self.gamma * (1 - done) + team_reward

		# Welford update per element
		batch_mean = return_val  # (batch_size,)
		batch_var = jnp.square(return_val - state.mean)  # elementwise variance estimate
		batch_count = jnp.ones_like(state.count)  # (batch_size,)
		...
		normed_reward = team_reward / jnp.sqrt(new_var + 1e-8)
		return obs, normed_reward[:, None], term, truncated, info, new_state
